...
kvalidate_config.inputErrorPosition = 'left';
	...
	this.triggerError = function(msg,name,displayNow)
	{
		...
		{	
			...
			if(this.submitErrorPosition == 'dialog')
			{
				...
				dialog.display('<div class="kformSubmitError">'+msg+'</div>');
			}
			else
			{
...
kdialog.prototype.display = function(dMsg)
{
	...
	var dialog = this;
	this.node.getElementsByClassName('kdialogXOut')[0].addEventListener('click',function(){dialog.hide()});
	...
	for(var i=0;i<nodes.length;i++)
		nodes[i].addEventListener('click',function(){dialog.hide()});
